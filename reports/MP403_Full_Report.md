...
This project investigates transcriptional differences between **Triple-Negative Breast (TNB)** and **Normal** tissue samples using a complete, end-to-end RNA-seq analysis workflow. The primary computational processing and statistical analyses were performed via the **Galaxy platform**, with downstream statistical analysis, visualization, ID conversion, and functional enrichment performed using **R** and **Bioconductor**.
...
RNA sequencing (RNA-seq) provides a high-throughput approach for investigating changes in gene expression between biological conditions. In this mini-project, RNA-seq data from TNB and Normal samples were analyzed to identify differentially expressed genes and characterize their associated biological functions and pathways.
...
$$\text{Raw FASTQ Reads} \longrightarrow \text{FastQC} \longrightarrow \text{fastp (Adapter \& Quality Trimming)} \longrightarrow \text{STAR (Splice-aware Alignment)} \longrightarrow \text{featureCounts} \longrightarrow \text{DESeq2} \longrightarrow \text{DEG Filtering} \longrightarrow \text{PCA / Volcano / Heatmap} \longrightarrow \text{GO / KEGG Functional Enrichment}$$
...
- **Quantification Matrix Dimension:** **78,733 genes $\times$ 6 samples** (File: `Galaxy136_count_matrix_78733_genes.tabular`).
...
- **PCA (Principal Component Analysis):** Reduced-dimensional representation based on normalized expression profiles.
- **Volcano Plot:** $\log_2\text{Fold Change}$ vs. $-\log_{10}(\text{padj})$ highlighting significantly modulated genes.
- **Heatmap:** Gene-wise Z-score standardized counts across ordered samples (`Normal_1, 2, 3` $\to$ `TNB_1, 2, 3`).

### 4.10 Functional Enrichment Analysis
- **Gene Ontology (GO):** Biological Process (BP), Molecular Function (MF), Cellular Component (CC).
- **KEGG Pathway Enrichment:** Identification of biological cascades and metabolic circuits.
- **ID Conversion:** Ensembl IDs mapped to Entrez Gene IDs via `org.Hs.eg.db` in R.
...
The volcano plot displays the distribution of differential expression magnitude against statistical significance.

![Figure 1: Volcano plot showing differential gene expression between TNB and Normal samples](../figures/94_DEG_volcano_plot.png)
...
![Figure 2: Principal Component Analysis of TNB and Normal samples](../figures/PCA_DEGs_TNB_vs_Normal.png)
...
![Figure 3: Heatmap of 94 differentially expressed genes across TNB and Normal samples](../figures/94_DEG_heatmap.png)
...
The top 20 most significant differentially expressed genes:
...
### 5.6 Gene Ontology (GO) Enrichment Analysis
- **Upregulated Genes:** Enriched in cell proliferation, mitotic cycle regulation, extracellular matrix disassembly, and cytokine receptor binding.
- **Downregulated Genes:** Enriched in normal mammary gland development, hormone response pathways, cell-cell adhesion, and lipid metabolic processes.

### 5.7 KEGG Pathway Enrichment Analysis
...
![Figure 4: KEGG pathway enrichment of upregulated genes](../figures/UP_KEGG_dotplot.png)

*Figure 4: KEGG pathway enrichment of upregulated genes.*

> [!NOTE]
> While the **TNF signaling pathway** showed the lowest nominal $p$-value among upregulated pathways, adjusted $p$-values were above 0.05. These findings represent exploratory pathway signals.
...
![Figure 5: KEGG pathway enrichment of downregulated genes](../figures/DOWN_KEGG_dotplot.png)

*Figure 5: KEGG pathway enrichment of downregulated genes.*

### 5.8 Downstream R Analysis & Identifier Conversion
- **Upregulated:** 23 out of 38 Ensembl IDs mapped to Entrez IDs via `org.Hs.eg.db`.
- **Downregulated:** 47 out of 56 Ensembl IDs mapped to Entrez IDs via `org.Hs.eg.db`.
